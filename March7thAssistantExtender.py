...
def kill_process(process_name):
    try:
        ...
        return False
####################仿真操作####################
def process(image_path,timeout,control,confidence=0.95,twoflag=False):
...
####################月卡领取####################
def month_card():
    if process("image/month_card/monthly_card.png",10,"click",confidence=0.8,twoflag=True)==True:
...
def tp_recovery():
    ...
    while error_time<=30:
        #检测有没有打开地图
        if process("image/tp_recovery/tp_map.png",3,"")==False:
            pyautogui.press("m")
        #检测是否已选择锚点
        elif process("image/tp_recovery/tp.png",0.5,"click",confidence=0.8)==True:
            logging.info(f"已传送至锚点附近")
            return
        #检测是否有可用的锚点
        elif process("image/tp_recovery/tp_place_black.png",0.5,"click",confidence=0.8)==True or process("image/tp_recovery/tp_place_white.png",0.5,"click",confidence=0.8)==True:
            logging.info(f"已选择锚点")
            process("image/tp_recovery/tp.png",0.5,"click")
            logging.info(f"已传送至锚点附近")
            return
        #通过大地图传送
        elif process("/image/tp_recovery/star_map.png",0.5,"click")==True:
            logging.info(f"打开星轨航图")
            process("image/tp_recovery/luofu.png",3,"click")
            logging.info(f"已选择仙舟罗浮地图")
            time.sleep(3)
            if process("image/tp_recovery/liuyundu_white.png",2,"click") or process("image/tp_recovery/liuyundu_black.png",2,"click")==True:
                logging.info(f"已选择流云渡地图")
            if process("image/tp_recovery/tp_place_white.png",2,"click") or process("image/tp_recovery/tp_place_black.png",2,"click")==True:
                logging.info(f"已选择传送锚点")
                process("image/tp_recovery/tp.png",1,"click")
                return
        else:
            error_time+=1
####################自动更新游戏####################
def main_game_auto_update():
    ...
    if process("image/game_auto_update/game_update.png",5,"click")==True:
        ...
        error_time=0
        while error_time<=60:
            if process("image/game_auto_update/game_in_download.png",0.5,"")==True:
                error_time=0
                logging.info(f"游戏内下载资源中")
                while True:
                    if process("image/game_auto_update/game_in_download.png",0.5,"")==False:
                        break
            elif process("image/game_auto_update/game_update_and_reboot.png",0.5,"")==True:
                error_time=0
                process("image/game_auto_update/yes.png",1,"click")
                kill_process("StarRail.exe")
                logging.info(f"游戏更新完成")
                return True
            elif process("image/game_auto_update/enter.png",0.5,"")==True or process("image/game_auto_update/login_other_account.png",0.5,"")==True:
                error_time=0
                logging.info(f"游戏内更新完成")
                kill_process("StarRail.exe")
                return True
            else:
                error_time+=1
                if error_time==60:
                    logging.error(f"游戏内更新超时")
                    return False
    else:
